...
if df_full.empty:
    st.error("❌ Error al cargar datos. Verifica el archivo Excel.")
    st.stop()
...
with tabs[0]:
    with st.container(border=True):
        ...
        st.plotly_chart(fig, use_container_width=True) if fig else st.info("Sin datos")
...
with tabs[5]:
    ...
    with c3, st.container(border=True):
        st.info("Espacio para métricas adicionales")
...
st.download_button("⬇️ Descargar CSV", csv, "fauna_export.csv", "text/csv")
