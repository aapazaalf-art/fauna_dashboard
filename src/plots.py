...
def apply_accessibility(fig):
    ...
    # Aplicar ejes solo si el gráfico los soporta
    try:
        ...
        pass  # Ignora gráficos sin ejes cartesianos (ej. pie, treemap)
    # Aplicar coloraxis solo si existe escala continua
    try:
        fig.update_coloraxes(tickfont=dict(color="#000000"), titlefont=dict(color="#000000"))
    except Exception:
        pass  # Ignora gráficos sin barra de color continua
    return fig
...
def plot_spatial_richness(df: pd.DataFrame):
    ...
    fig = apply_accessibility(fig)
    st.plotly_chart(fig, use_container_width=True)
    st.caption("📌 Representa la diversidad observada por ecozona.")
